...
import { SHADOW_FRAME_ALLOWED_TAGS } from './tags';
import { SHADOW_FRAME_ALLOWED_ATTRIBUTES } from './attributes';

export const sanitizeDom = (): Transformer => {
  return dom => {
    const sanitizedHtml = sanitizeHtml(dom.innerHTML, {
      allowedTags: SHADOW_FRAME_ALLOWED_TAGS,
      allowedAttributes: SHADOW_FRAME_ALLOWED_ATTRIBUTES,
