...
import { buildMkDocsConfig } from './buildConfig';
...
export async function createMkDocsConfig({
  directory,
  overwrite = true,
}: Options) {
  ...
  // TODO - support dotfile config for user
  const config = await getConfig();
  const mkdocsConfig = config.getConfig('techdocs.mkdocs.config');

  const contents = buildMkDocsConfig({ config: mkdocsConfig });
